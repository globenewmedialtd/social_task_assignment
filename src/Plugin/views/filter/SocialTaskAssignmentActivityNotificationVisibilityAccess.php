...
use Drupal\activity_viewer\Plugin\views\filter\ActivityNotificationVisibilityAccess;
 ...
 *
 * @ViewsFilter("social_task_assignment_activity_notification_visibility_access")
 */
class SocialTaskAssignmentActivityNotificationVisibilityAccess extends ActivityNotificationVisibilityAccess {

  /**
   * Not exposable.
   */
  public function canExpose() {
    return parent::canExpose();
  }
  ...
  public function query() {
    ...
    // activities are only created for those with the TaskOrganizerActivityContext
    ...
    if ($authenticated) {
      $task_assignment_access = new Condition('AND');
      $task_assignment_access->condition('activity__field_activity_entity.field_activity_entity_target_type', 'task_assignment');
      $task_assignment_access->condition('activity__field_activity_recipient_user.field_activity_recipient_user_target_id', (string) $account->id());
      $or->condition($task_assignment_access);
    }     
