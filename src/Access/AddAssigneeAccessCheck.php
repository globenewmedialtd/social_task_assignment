 ...
 */
class AddAssigneeAccessCheck implements AccessInterface {
  ...
  public function access(Route $route, RouteMatchInterface $route_match, AccountInterface $account) {
    $permission = $route->getRequirement('_assignee_permission');

    // Don't interfere if no permission was specified.
    // Or it's not the permission we are looking for.
    if ($permission === NULL || $permission !== 'manage everything assignments') {
    ...
    $managers_access = SocialTaskAssignmentAccessHelper::getEntityAccessResult($node, 'update', $account);
