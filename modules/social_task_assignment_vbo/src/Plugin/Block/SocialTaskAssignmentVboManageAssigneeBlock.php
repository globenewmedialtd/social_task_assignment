...
/**
 * Provides a 'SocialTaskAssignmentVboManageAssigneeBlock' block.
 *
 * @Block(
 *  id = "social_task_assignment_vbo_manage_assignee_block",
 *  admin_label = @Translation("Social Task Assignment VBO manage assignee block"),
 * )
 */
class SocialTaskAssignmentVboManageAssigneeBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    if (isset($task)) {
      $link = [
        '#type' => 'link',
        '#title' => $this->t('Add new assignees'),
        '#weight' => 1,
        '#url' => Url::fromRoute('view.task_assignment_selection_source.page',['node' => $task->id()]),
      ...
      $build['#cache'] = [
        'keys' => ['social_task_assignment_vbo_manage_assignee_block', 'node', $task->id()],
        'contexts' => ['user'],
      ];
