...
namespace Drupal\social_task_assignment_vbo\Controller;
...
/**
 * Class SocialTaskAssignmentVboController.
 *
 * @package Drupal\social_task_assignment_vbo\Controller
 */
class SocialTaskAssignmentVboController extends ControllerBase {
  ...
  public function access(AccountInterface $account, RouteMatch $route_match) {
    if ($account->hasPermission('manage everything assignments')) {
      ...
      if ($node instanceof NodeInterface && $node->bundle() === 'task' && !$node->field_task_managers->isEmpty()) {
